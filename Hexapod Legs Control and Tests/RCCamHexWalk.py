...
#Gamepad settings
axisUpDown = 1 #left stick up/down
axisUpDownInverted = True 
camAxisUpDown = 4 #right stick up/down
camAxisUpDownInverted = True
axisLeftRight = 3 #right stick left/right
axisLeftRightInverted = False
camAxisLeftRight = 0 #left stick left/right
camAxisLeftRightInverted = False
interval = 0.1
...
def lookLeft():
	call("echo 3=100 > /dev/servoblaster", shell=True)

def lookRight():
	call("echo 3=200 > /dev/servoblaster", shell=True)

def lookForward():
	call("echo 3=150 > /dev/servoblaster", shell=True)
...
def PygameHandler(events): #Gets joystick values and translates it to a movement
    ...
    for event in events:
        if event.type == pygame.JOYAXISMOTION:
            ...
            camUpDown = joystick.get_axis(camAxisUpDown)
            camLeftRight = joystick.get_axis(camAxisLeftRight)
            print(upDown)
            print(leftRight)
            print(camUpDown)
            print(camLeftRight)
    ...
    while True:
        ...
        if hadEvent:
            ...
            if moveForward:
            	...
            	lookRight()
            elif camStraight and camForward:
            	lookForward()
            	lookStraight()
        time.sleep(interval)
